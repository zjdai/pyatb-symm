...
for candidate in (
    ...
    if candidate and os.path.isdir(candidate) and candidate not in include_dirs:
...
for candidate in (
    ...
    if candidate and os.path.isdir(candidate) and candidate not in library_dirs:
        library_dirs.append(candidate)

if not any(os.path.exists(os.path.join(path, "libopenblas.so")) for path in library_dirs):
    if any(os.path.exists(os.path.join(path, "libblas.so")) for path in library_dirs):
