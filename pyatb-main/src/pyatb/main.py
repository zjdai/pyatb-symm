...
from pyatb.io.abacus_read_stru import read_stru as read_abacus_stru
...
from pyatb.symmetry import Character, calculate_kp_irreps
from pyatb.transport import *
import os
from pathlib import Path


def _validate_character_basis_files(stru_file: str) -> None:
    stru_atoms = read_abacus_stru(stru_file)
    missing_records = []
    seen = set()

    for atom in stru_atoms:
        pseudo_path = getattr(atom, "pseudo_file", None)
        if pseudo_path is not None:
            pseudo_path = Path(pseudo_path).resolve()
            key = ("pseudo", str(pseudo_path))
            if key not in seen and not pseudo_path.exists():
                seen.add(key)
                missing_records.append(("pseudo", atom.species, str(pseudo_path)))

        orbital_path = Path(getattr(atom, "orb_file")).resolve()
        key = ("orbital", str(orbital_path))
        if key not in seen and not orbital_path.exists():
            seen.add(key)
            missing_records.append(("orbital", atom.species, str(orbital_path)))

    if missing_records:
        if RANK == 0:
            with open(RUNNING_LOG, "a", encoding="utf-8") as fp:
                fp.write("\nCharacter basis-file check failed\n")
                fp.write(f"STRU = {Path(stru_file).resolve()}\n")
                fp.write("Missing files:\n")
                for file_kind, species, file_path in missing_records:
                    fp.write(f"  - [{file_kind}] species={species} path={file_path}\n")
        details = ", ".join(f"{kind}:{path}" for kind, _, path in missing_records)
        raise FileNotFoundError(f"Missing pseudopotential/orbital files for CHARACTER: {details}")
...
def main():
    ...
    if function_switch['CHARACTER']:
        character_parameters = dict(INPUT['CHARACTER'])
        stru_file = character_parameters.get('stru_file', 'STRU')
        _validate_character_basis_files(stru_file)
        cal_character = Character(m_tb)
        cal_character.calculate_character(
    ...
    if function_switch['KP']:
        kp_parameters = dict(INPUT['KP'])
        stru_file = kp_parameters.get('stru_file', 'STRU')
        _validate_character_basis_files(stru_file)
        calculate_kp_irreps(
            m_tb,
            **kp_parameters,
